...
1.2.0 将首页进一步做成可长期使用的音乐入口：每日推荐现已支持网易云、QQ 音乐和酷狗音乐，并与右侧 3D 歌单架同步；首页六个卡片槽位支持右键移除、空位加号和固定个人歌单。

## 当前版本

当前版本：`1.2.0`

状态：1.2.0 三平台每日推荐与可自定义首页正式版。
...
正式分发以 Release 资产中的 `Mineradio-1.2.0-Setup.exe` 为准，不建议直接下载 `win-unpacked` 目录作为正式分发包。安装包会创建桌面快捷方式；直接运行打包版 `Mineradio.exe` 时，应用也会在首次启动时补创建桌面快捷方式。
