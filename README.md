...
1.1.3 重点完善首页信息与播放出口：天气会在定位后快速显示，歌曲热评每 30 秒随机更新；三平台音质支持优先档位与逐级降级，并可选择 Mineradio、Windows 默认应用、网易云音乐或自选 EXE 作为默认播放软件。

## 当前版本

当前版本：`1.1.3`

状态：1.1.3 天气、热评、音质与外部播放器正式版。
...
正式分发以 Release 资产中的 `Mineradio-1.1.3-Setup.exe` 为准，不建议直接下载 `win-unpacked` 目录作为正式分发包。安装包会创建桌面快捷方式；直接运行打包版 `Mineradio.exe` 时，应用也会在首次启动时补创建桌面快捷方式。
