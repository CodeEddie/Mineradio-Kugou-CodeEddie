...
1.4.0 在底部播放器新增定时暂停功能，提供多档快捷时长和 1–480 分钟自定义设置；同时重做程序退出清理，关闭时会主动释放渲染、音频、摄像头、Worker 和本地服务资源。

## 当前版本

当前版本：`1.4.0`

状态：1.4.0 定时暂停与退出清理正式版。

> 安全提示：`v1.0.10` 及更早旧安装包不再建议继续安装或传播，请先隔离旧安装包。本次不提供从 `v1.0.10` 到 `v1.1.0` 的软件内本地更新，请在 GitHub Release 页面手动下载 `v1.1.0` 安装包进行纯净安装。

## QQ 音乐播放限制

QQ 音乐登录成功、能够读取账号资料和个人歌单，不代表所有歌曲都能在 Mineradio 内直接播放。部分版权歌曲只允许 QQ 音乐官方客户端取得音频地址；这类歌曲即使在官方客户端可以正常播放，QQ 接口仍可能向第三方播放器返回 `104003`。

- 该限制常见于周杰伦、林俊杰等版权曲目，与歌单内容或歌曲 `mid` 丢失无关。
- Mineradio 会尝试账号允许的音质档位，但无法绕过 QQ 音乐的官方客户端授权和版权策略。
- 遇到 `104003` 时，Mineradio 会暂停播放并常驻显示失败原因，不会继续逐首尝试整个歌单，也不会擅自换成其他平台的翻唱版本。
- 需要播放此类歌曲时，请使用 QQ 音乐官方客户端。

## 下载与安装

[下载最新 Windows 安装包](https://github.com/CodeEddie/Mineradio-Kugou-CodeEddie/releases/latest/download/Mineradio-1.4.0-Setup.exe) · [下载免安装便携版](https://github.com/CodeEddie/Mineradio-Kugou-CodeEddie/releases/latest/download/Mineradio-1.4.0-Portable.zip) · [查看全部版本](https://github.com/CodeEddie/Mineradio-Kugou-CodeEddie/releases)

| 版本 | Windows 安装包 | 便携版 / 源码 | 更新说明 |
| --- | --- | --- | --- |
| `v1.4.0` | [下载安装包](https://github.com/CodeEddie/Mineradio-Kugou-CodeEddie/releases/download/v1.4.0/Mineradio-1.4.0-Setup.exe) | [便携 ZIP](https://github.com/CodeEddie/Mineradio-Kugou-CodeEddie/releases/download/v1.4.0/Mineradio-1.4.0-Portable.zip) | [Release](https://github.com/CodeEddie/Mineradio-Kugou-CodeEddie/releases/tag/v1.4.0) |
...
- 软件内动态背景预设，支持星河、极光、日蚀、星图等音乐律动粒子视觉，不修改 Windows 系统壁纸
...
- 底部播放器支持定时暂停，可选择快捷时长、输入自定义分钟数并实时查看剩余时间
- 统一退出流程会主动停止渲染、音频、摄像头、Worker、计时器和本地服务，降低反复启动关闭时的资源叠加
...
```powershell
tar -a -cf dist/Mineradio-1.4.0-Portable.zip -C dist/win-unpacked .
```

生成 SHA256：

```powershell
Get-FileHash dist/Mineradio-1.4.0-Setup.exe -Algorithm SHA256
Get-FileHash dist/Mineradio-1.4.0-Portable.zip -Algorithm SHA256
```
...
<details open>
<summary><strong>v1.4.0 — 底部播放器定时暂停</strong></summary>

- 新增底栏时钟入口和暗色定时设置面板。
- 支持 15、30、45、60、90、120 分钟快捷设置。
- 支持 1–480 分钟自定义时长与随时取消。
- 底栏显示剩余分钟，面板显示精确到秒的倒计时。
- 到时沿用现有音频淡出链路暂停播放。
- 关闭程序时主动释放前端媒体、GPU 渲染和本地服务资源，并提供超时退出兜底。
- 右上角版本入口可直接查看本次完整更新日志。

</details>

<details open>
<summary><strong>v1.3.0 — 软件内动态背景预设与双行歌词</strong></summary>

- “预设与存档”新增极光、日蚀、星图三套软件内动态背景。
- 动态背景与 Emily 专辑封面、星河、唱片等现有视觉并列选择，并跟随封面取色及音乐频段律动。
- 所有新增视觉只在 Mineradio 内部粒子舞台显示，不修改 Windows 系统壁纸。
