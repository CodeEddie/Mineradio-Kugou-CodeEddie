...
- 正式开放 Windows 动态壁纸模式，将 Mineradio 音乐视觉挂载到桌面壁纸层，并保留歌曲封面、播放状态和视觉配色联动。
- 新增内置 Wallpaper Library，提供“极光脉冲、日蚀光环、黑胶唱片、星图连线”四套动态壁纸模板，支持一键切换与透明度调节。
- 动态壁纸模板、开关和透明度会随本地视觉布局及用户视觉存档保存，重启后可恢复上次选择。
- 舞台歌词升级为双行结构：当前演唱歌词保持高亮，下一句歌词显示在下方并弱化呈现。
- 歌词设置新增“未播放歌词透明度”和“歌词句间距”，支持实时调整、重启恢复和用户存档。
- 优化壁纸画布清屏与光晕能量，避免长时间运行后亮度累积过曝，同时保持暗色层次和桌面图标可读性。
