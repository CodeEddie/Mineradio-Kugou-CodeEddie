...
- 底部播放器新增定时暂停入口，支持 15、30、45、60、90、120 分钟快捷设置。
- 支持输入 1–480 分钟的自定义时长，并可随时取消当前定时。
- 定时启用后，底栏显示剩余分钟，设置面板显示精确到秒的倒计时。
- 到达设定时间后沿用播放器现有淡出链路暂停播放，并同步播放图标与状态。
- 使用绝对结束时间计算倒计时，电脑短暂休眠或页面卡顿后仍会按真实时间执行。
- 重做程序退出链路：关闭窗口时先停止前端渲染、音频、摄像头、Worker 和关键计时器，再关闭本地 HTTP 连接与桌面歌词轮询器，最后退出 Electron，并提供超时强制退出兜底。
- 修复打包版未包含 `CHANGELOG.md` 导致点击右上角版本号后无法显示当前版本更新日志的问题。

## v1.3.0

- 搜索新增酷狗音乐 `KG` 独立页签，`All` 现在会并行检索并合并网易云、QQ 音乐和酷狗音乐结果。
- QQ 音乐返回 `104003` 时不再逐首换源或自动跳过整个歌单；播放器会立即暂停，并在右上角常驻显示失败歌曲、原因和错误码，直到用户手动关闭。
- 新增软件内版本号入口与当前版本更新日志窗口，内容直接读取 `package.json` 和 `CHANGELOG.md`。
- “预设与存档”新增软件内动态背景：极光、日蚀、星图，与 Emily 专辑封面、星河和唱片等视觉预设并列选择。
- 新增动态背景继续使用 Mineradio 内部 Three.js 粒子舞台，不修改 Windows 系统壁纸；支持封面取色、播放状态及音乐频段联动。
- 极光使用缓慢漂移的流光丝带，日蚀使用随低频呼吸的多层光环，星图使用随高频闪烁的深空星点。
- 舞台歌词升级为双行结构：当前演唱歌词保持高亮，下一句歌词显示在下方并弱化呈现。
- 歌词设置新增“未播放歌词透明度”和“歌词句间距”，支持实时调整、重启恢复和用户存档。
- 优化动态背景的粒子透明度与光晕能量，保持暗色层次并避免长时间播放后亮度过曝。

## v1.2.0

- 首页“每日推荐”升级为网易云、QQ 音乐、酷狗音乐三平台入口：网易云继续使用账号每日推荐，QQ 与酷狗根据用户个人歌单生成每天稳定更新的今日歌曲。
- 每日推荐支持“全部来源 / 网易云 / QQ / 酷狗”切换，播放队列、封面、歌曲数量和来源提示会随选择同步更新。
- 右侧 3D 歌单架新增“每日推荐”卡片，可直接播放今日队列，也可打开详情浏览推荐歌曲；切换每日来源后歌单架会同步刷新。
- 首页右侧六格改为可持久化槽位：支持右键或 `Shift+F10` 从首页移除卡片，空位会显示加号。
- 首页加号面板可重新添加系统功能，也可从已登录的网易云、QQ 音乐或酷狗音乐个人歌单中选择歌单固定到首页。
- 优化首页六格刷新策略，天气、热评等后台更新不再反复重建卡片，减少不必要的闪烁和交互中断。
